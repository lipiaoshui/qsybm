		...
		//添加数据
		 for (int index=0;index<propertyNames.length;index++)
		 {
		     ...
		     else if (DataBaseConstant.CREATE_BY.equals(propertyNames[index]))
		     {
		         /*使用拦截器将对象的"创建人"属性赋上值*/
		    	 if(oConvertUtils.isEmpty(state[index])){
		    		  state[index] = ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_ID);
		    	 }
		         continue;
		     }
		     /*找到名为"创建人名称"的属性*/
		     ...
		     /*找到名为"有效标志"的属性*/
		     if (DataBaseConstant.ENABLE_FLAG.equals(propertyNames[index])
		    		 ||DataBaseConstant.ENABLE_FLAG_TABLE.equals(propertyNames[index]))
		     {
		         /*使用拦截器将对象的"有效标志"属性赋上值*/
		    	 if(oConvertUtils.isEmpty(state[index])){
		    		 state[index] = "1";
		    	 }
		         continue;
	...
	//添加数据
     for (int index=0;index<propertyNames.length;index++)
     {
         ...
         else if (DataBaseConstant.UPDATE_BY.equals(propertyNames[index]))
         {
             /*使用拦截器将对象的"修改人"属性赋上值*/
        	 if(oConvertUtils.isEmpty(currentState[index])){
        		 currentState[index] = ResourceUtil.getUserSystemData(DataBaseConstant.SYS_USER_ID);
        	 }
        	 continue;
         }
         /*找到名为"修改人名称"的属性*/
