...
public class RestAuthTokenInterceptor implements  HandlerInterceptor {
	...
	@Resource(name="customTokenManager")
...
//	}
	...
	public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object obj) throws Exception {
		...
		TokenModel model = manager.getToken(authHeader, null);
		if (manager.checkToken(model)) {
			//如果token验证成功，将token值放入request中，便于之后注入
			request.setAttribute(JwtConstants.CURRENT_TOKEN_VALUE, authHeader);
			return true;
		} else {
			// 如果验证token失败，则返回401错误
			response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
