...
//          sql = sql.replace("${" + key + "}", "'"+String.valueOf(params.get(key))+"'");
	...
	private void fillInsertSysVar(String tableName,Map<String, Object> data) {
		...
		if(data.containsKey(DataBaseConstant.INST_CODE_TABLE)
				||getAllFieldByTableName(tableName).containsKey(DataBaseConstant.INST_CODE_TABLE)){
			data.put(DataBaseConstant.INST_CODE_TABLE, ResourceUtil.getUserSystemData(DataBaseConstant.INST_CODE));
		}
	}
