...
//			systemService.saveOrUpdate(readUser);
	...
	public void datagrid2(TSNotice tSNotice,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
		if(!StringUtil.isEmpty(tSNotice.getCreateUser())){
			tSNotice.setCreateUser("*"+tSNotice.getCreateUser()+"*");
		}
		if(!StringUtil.isEmpty(tSNotice.getNoticeTitle())){
			tSNotice.setNoticeTitle("*"+tSNotice.getNoticeTitle()+"*");
		}	

		CriteriaQuery cq = new CriteriaQuery(TSNotice.class, dataGrid);
		tSNotice.setRecordState("1");
		//查询条件组装器
	...
	@ResponseBody
	public AjaxJson doDel(TSNotice tSNotice,String pageInstCode, HttpServletRequest request) {
		String message = null;
		AjaxJson j = new AjaxJson();
...
/*			if("2".equals(tSNotice.getNoticeLevel())){
			...
			//String sql = "UPDATE t_s_notice SET record_state = '2' WHERE id =?";
	        //判断当前用户能否删除当前信息
//			if(instCode.equals(pageInstCode)){
//				
//				String sql = "delete from t_s_notice   WHERE id =?";
//				systemService.executeSql(sql,tSNotice.getId());
...
//			}
	...
	public AjaxJson doAdd(TSNotice tSNotice, HttpServletRequest request) {
		...
		tSNotice.setCreateUser(orgUser.get(0).getTsUser().getUserName());
		tSNotice.setUpdateTime(new Date());
		try{
			if(null != tSNotice&& StringUtil.isNotEmpty(tSNotice.getId())){
