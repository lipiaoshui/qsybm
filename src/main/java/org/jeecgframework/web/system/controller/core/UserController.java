...
public class UserController extends BaseController {
	...
	public String user(HttpServletRequest request) {
		...
		departList.clear();
//		//控制机构下拉框的权限
//		String id = ResourceUtil.getSessionUser().getId();
//        List<TSUserOrg> orgUser = systemService.findByProperty(TSUserOrg.class, "tsUser.id", id);
//        String orgCode = orgUser.get(0).getTsDepart().getOrgCode();
//        request.setAttribute("orgCode", orgCode.length());
...
//			}
	...
	public void datagrid(TSUser user,HttpServletRequest request, HttpServletResponse response, DataGrid dataGrid) {
        ...
        // 获取当前登录用户的组织机构编号进行权限过滤
        String orgCode = ResourceUtil.getSessionUser().getCurrentDepart().getOrgCode();
        List<String> orgCodeList = systemService.findByQueryString("select uo.tsUser.id from TSUserOrg uo,TSDepart d where uo.tsDepart.id=d.id and d.orgCode like '"+orgCode+"%'");
        cq.in("id", orgCodeList.toArray());
...
//		// 获得机构类型
//		String sql = "select inst_type from xjbm_base_inst where org_code = "
//				+ "(select org_code from t_s_depart where id = ? )";
//		String instType = jdbcDao.queryForObject(sql, String.class, orgid);
//		// 获取当前用户的机构类型
//		sql = "select inst_type from xjbm_base_inst where inst_code = ?";
//		String userInstType = jdbcDao.queryForObject(sql, String.class, ResourceUtil.getSessionUser().getInstCode());
//		// 如果是当前用户是【监管机构】，创建用户并非【监管机构】时，修改相关APP角色
//		if(!Globals.INST_TYPE_SUPERVISE.equals(instType) && Globals.INST_TYPE_SUPERVISE.equals(userInstType)){
//			// 获取APP角色
//			String appRoleCode = user.getAppRoleCode();
//			if("01".equals(appRoleCode)){
//				sql = "select role_code from xjbm_app_role_info where role_name = '管理员' and inst_type = ?";
//				user.setAppRoleCode(jdbcDao.queryForObject(sql, String.class, instType));
//			}else{
//				sql = "select role_code from xjbm_app_role_info where role_name = '巡检员' and inst_type = ?";
//				user.setAppRoleCode(jdbcDao.queryForObject(sql, String.class, instType));
//			}
//		}
		if (StringUtil.isNotEmpty(user.getId())) {
...
//			users.setDataAuth(user.getDataAuth());
//			users.setAppRoleCode(user.getAppRoleCode());
...
//				user.setStaffCode(buildStaffCode());
//				
//				String orgAttr[] = orgid.split(",");
//				TSDepart dp =systemService.getEntity(TSDepart.class, orgAttr[0]);
//				List<XjbmBaseInstEntity> list = systemService.findByProperty(XjbmBaseInstEntity.class, "orgCode", dp.getOrgCode());
///*				String instCode = ResourceUtil.getSessionUser().getInstCode();
//*/				user.setInstCode(list.get(0).getInstCode());
...
//	@RequestMapping(params = "batchAddUser")
//	@ResponseBody
//	public AjaxJson batchAddUser(HttpServletRequest request) {
//		String message = null;
//		AjaxJson j = new AjaxJson();
//
//		// 读取Excel数据
...
//						String password = df.format(row.getCell(2).getNumericCellValue());
//						String mobilePhone = df.format(row.getCell(9).getNumericCellValue());
//
//						String devFlag = "0";
//						String dataAuth = "1";
//						String appRoleCode = "06";
//					
//						Short logType=Globals.Log_Type_UPDATE;
//						String roleid = "40289d8164d0268f0164d02a27750001";
//						String orgid= "aafbfd6d671bd5e101673113ad730063";
//						
//						TSUser user = new TSUser();
//						user.setMobilePhone(mobilePhone);
//						user.setDevFlag(devFlag);
//						user.setUserName(username);
//						user.setRealName(realname);
//						user.setDataAuth(dataAuth);
//						user.setAppRoleCode(appRoleCode);
//						
//						TSUser users = systemService.findUniqueByProperty(TSUser.class, "userName",user.getUserName());
//						if (users != null) {
//							message = "用户: " + users.getUserName() + "已经存在";
//						} else {
//							user.setPassword(PasswordUtil.encrypt(username, oConvertUtils.getString(password), PasswordUtil.getStaticSalt()));
//							user.setStatus(Globals.User_Normal);
//							user.setDeleteFlag(Globals.Delete_Normal);
//							
//							user.setStaffCode(buildStaffCode());
...
//				message = "用户: " + users.getUserName() + "已经存在";
//			} else {
//				user.setPassword(PasswordUtil.encrypt(user.getUserName(), oConvertUtils.getString(request.getParameter("password")), PasswordUtil.getStaticSalt()));
//				user.setStatus(Globals.User_Normal);
//				user.setDeleteFlag(Globals.Delete_Normal);
//				
//				user.setStaffCode(buildStaffCode());
...
//		systemService.addLog(message, logType, Globals.Log_Leavel_INFO);*/
//		j.setMsg(message);
//		logger.info("["+IpUtil.getIpAddr(request)+"][添加编辑用户]"+message);
//		return j;
//
//	}
	
	public String buildStaffCode() {
		Calendar cal = Calendar.getInstance();
		int year = cal.get(Calendar.YEAR);
		StringBuilder codePrefix = new StringBuilder(Globals.CODE_PREFIX_STAFF);
		codePrefix.append(year % 100);
		StringBuilder sql = new StringBuilder("select IFNULL(max(cast(staff_code as decimal(10))),0) as maxcode from t_s_base_user");
		sql.append(" where staff_code like '").append(codePrefix).append("%'");
		long maxcode = this.systemService.getCountForJdbc(sql.toString());
		if (maxcode > 0) {
			return String.valueOf(maxcode + 1);
		}
		return codePrefix.append("0001").toString();
	}
    /**
     * 保存 用户-组织机构 关系信息
...
//        String instCode = ResourceUtil.getSessionUser().getInstCode();
//        XjbmBaseInstEntity instBean = systemService.findUniqueByProperty(XjbmBaseInstEntity.class, "instCode", instCode);
		if (StringUtil.isNotEmpty(user.getId())) {
			user = systemService.getEntity(TSUser.class, user.getId());
...
//			instBean = systemService.findUniqueByProperty(XjbmBaseInstEntity.class, "instCode", user.getInstCode());
		}
		req.setAttribute("tsDepart", tsDepart);
//		req.setAttribute("instCode", instCode);
//		req.setAttribute("instType", instBean.getInstType());
...
//                saveUserOrgList(req, user);
	...
	public void idandname(HttpServletRequest req, TSUser user) {
		...
		//针对只能选择一个角色
		String [] roleAttr = roleId.split(",");
		if(roleAttr.length ==1){
			roleId = roleAttr[0];
		}
		//针对只能选择一个角色结束
...
//			}
	...
	public void exportXls(TSUser tsUser,HttpServletRequest request,HttpServletResponse response
...
/*		CriteriaQuery cq = new CriteriaQuery(TSUser.class, dataGrid);
		...
		beanList .add(new ExcelExportEntity("账号", "username",15));
		beanList .add(new ExcelExportEntity("姓名", "realname",15));
	//	beanList .add(new ExcelExportEntity("进校日期", "registrationDate"));
		StringBuilder sb = new StringBuilder();
		sb.append(" SELECT username,realname from t_s_base_user  u WHERE u.delete_flag= 0");
		if(!StringUtil.isEmpty(tsUser.getUserName())){
			sb.append(" and u.username like '%").append(tsUser.getUserName()).append("%' ");
		}
		if(!StringUtil.isEmpty(tsUser.getRealName())){
			sb.append(" and u.realname like '%").append(tsUser.getRealName()).append("%' ");
		}
		System.out.println(sb.toString());
		List<Map<String, Object>> list = systemService.findForJdbc(sb.toString());
		 try {
			 	ServletOutputStream outputStream = response.getOutputStream();
				if (list.size() < 1) {
					response.setContentType("text/html");
					response.setCharacterEncoding("UTF-8");
					String content = "<script type='text/javascript'>alert('无数据可导出');history.go(-1);</script>";
					outputStream.write(content.getBytes("UTF-8"));
					return;
				}

				Workbook workbook = ExcelExportUtil.exportExcel(new ExportParams("用户表列表", "导出人:"+ResourceUtil.getSessionUser().getRealName()), beanList ,list);
				response.reset();// 清空输出流
				
				
				
				
				String fileName = new String("用户表.xls".getBytes(),"ISO8859-1");
...
/*		modelMap.put(NormalExcelConstants.PARAMS,new ExportParams("用户表列表", "导出人:"+ResourceUtil.getSessionUser().getRealName(),
				"导出信息"));*/
		//modelMap.put(NormalExcelConstants.DATA_LIST,workbook);
