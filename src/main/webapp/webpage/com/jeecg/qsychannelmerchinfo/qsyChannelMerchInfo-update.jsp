...
<t:formvalid formid="formobj" dialog="true" usePlugin="password" layout="table" beforeSubmit="vaild" action="qsyChannelMerchInfoController.do?doUpdate">
	...
	<table style="width: 600px;" cellpadding="0" cellspacing="1" class="formtable">
		<tr>
			...
			<td class="value" width="10%">
                <t:dictSelect field="channelCode" dictTable="qsy_channel_info" dictField="channel_code" dictText="channel_name" id="channelCode" defaultVal="${qsyChannelMerchInfo.channelCode}" readonly="readonly" datatype="*"></t:dictSelect>
		...
		<tr>
			...
			<td class="value" width="10%">
                <input id="merchCode" type="hidden" class="inputxt" value="${qsyChannelMerchInfo.merchCode}" name="merchCode" />
                <input id="merchName" name="merchName" class="inputxt" value="${merchName}" readonly="readonly" />
...
<script type="text/javascript">
	$(function(){
		$("#channelMerchCode").live('change',function(){
			if($("#merchCode").val()!=null && $.trim($("#merchCode").val())!='' && $("#channelCode").val()!=null && $.trim($("#channelCode").val())!='' && $("#channelCode").val()!=channelCode){
				vaildMerch();
			}
		})
	...
	function validPass(bool,htmlText){
		if(bool){
			flag = true;
			$("#merchName").removeClass("Validform_error");
			...
			$("#merchName").addClass("Validform_error");
			$("#vaildChannel").addClass("Validform_wrong");
			$("#vaildChannel").html(htmlText);
		}
	}
	var merchCode = $("#merchCode").val();
	var channelMerchCode = $("#channelMerchCode").val();
	var flag = true;
	function vaild(){
		if($("#merchCode").val()==null || $.trim($("#merchCode").val())==''){
			validPass(false,"所属商户不能为空");
		}
		if($("#channelMerchCode").val()==channelMerchCode && merchCode == $("#merchCode").val()){
			$("#merchName").removeClass("Validform_error");
			$("#vaildChannel").removeClass("Validform_wrong");
			$("#vaildChannel").addClass("Validform_right");
			$("#vaildChannel").html("通过信息验证！");
